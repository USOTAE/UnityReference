...
public class Inventory : MonoBehaviour, ISaveManager
{
    ...
    private void Start()
    {
        ...
        //AddStartingItems();
        StartCoroutine(WaitLoadGameAddStartingItems());
    }

    private IEnumerator WaitLoadGameAddStartingItems()
    {
        yield return null;
        AddStartingItems();
    }
    ...
    {
        //Debug.Log("Inventory AddStartingItems");
    ...
    public bool CanCraft(ItemData_Equipment _itemToCraft, List<InventoryItem> _requirements)
    {
        List<InventoryItem> materialsToRemove = new List<InventoryItem>();

        for (int i = 0; i < _requirements.Count; i++)
        {
            if (stashDictionary.TryGetValue(_requirements[i].data, out InventoryItem stashValue))
            {
                if (stashValue.stackSize < _requirements[i].stackSize)
                {
        ...
        {
            Debug.Log("remove materials + " + materialsToRemove[i].data.name);
